...
export default function App({ data, source, refresh, backend }: { data?: PilotData; source?: 'supabase' | 'offline'; refresh?: () => void; backend?: BackendHealth }) {
  const [section, updateSection] = useState<Section>(getSection)
  const [mobile, setMobile] = useState(false)
  const [auth, setAuth] = useState(false)
  const [info, setInfo] = useState<'help' | 'notifications' | 'donation' | null>(null)
  ...
  const callback = useSWR(code ? ['auth-callback', code] : null, async ([, callbackCode]) => {
  ...
  return <div className="app-shell"><a className="skip-link" href="#main-content">Skip to content</a><AppSidebar section={section} setSection={setSection} mobileOpen={mobile} close={() => setMobile(false)} session={session} signIn={() => setAuth(true)} signOut={signOut} activeCount={active?.length} help={() => setInfo('help')} /><div className="app-main"><header className="topbar"><div className="breadcrumb"><Button variant="ghost" size="icon" className="mobile-menu" aria-label="Open navigation" onClick={() => setMobile(true)}><Menu /></Button><span>Workspace</span><ChevronRight size={13} /><strong>{sectionNames[section]}</strong></div><div className="topbar-actions"><button className="location-button" onClick={() => setSection('settings')}><MapPin size={14} /><span>Bengaluru, IN</span><ChevronDown size={12} /></button><span className="topbar-divider" /><Button variant="ghost" size="icon" aria-label="Safety guidelines" onClick={() => setInfo('help')}><CircleHelp /></Button><Button variant="ghost" size="icon" aria-label="View notifications" onClick={() => setInfo('notifications')}><Bell /></Button><button className="topbar-avatar" onClick={() => setAuth(true)} aria-label={session ? 'Account' : 'Sign in'}>{session ? 'RP' : <Leaf size={17} />}</button></div></header>
    ...
    {!data && <div className="setup-banner"><span className="setup-banner-icon"><Database size={16} /></span><p><strong>A fresh start for the Bengaluru pilot.</strong><span>Supabase is connected. Database setup is awaiting approval.</span></p><button onClick={() => setSection('settings')}>View setup<ChevronRight size={14} /></button></div>}
    {section === 'overview' && <div className="overview-content"><OverviewMetrics data={data} /><div className="overview-middle"><RescueMap data={data} expanded={expandedMap} onExpand={() => setExpandedMap(!expandedMap)} /><ActivityFeed data={data} navigate={setSection} /></div>{!!urgent?.length && <div className="urgency-banner"><ShieldCheck size={17} /><p><strong>{urgent.length} {urgent.length === 1 ? 'rescue needs' : 'rescues need'} attention.</strong> Less than an hour remains in the safe window.</p><Button variant="ghost" size="sm" onClick={() => setSection('dispatch')}>Review rescues<ChevronRight data-icon="inline-end" /></Button></div>}<DonationsTable data={data} now={now} openDonation={setSelected} viewAll={() => setSection('donations')} /></div>}
    {section === 'donations' && <DonationsTable data={data} now={now} full openDonation={setSelected} viewAll={() => {}} />}
    {section === 'dispatch' && <DispatchView data={data} now={now} openDonation={setSelected} />}
    ...
    {section === 'settings' && <SettingsView ready={!!data} />}
    <footer className="page-footer"><span><Leaf size={13} />Made for a world with less waste.</span><span>Surplus to Shelter <i>·</i> Bengaluru pilot</span></footer>
    </main></div>
    <AuthDialog open={auth} onOpenChange={setAuth} /><DonationDialog donation={selected} data={data} now={now} close={() => setSelected(null)} />
    <Dialog open={info !== null} onOpenChange={open => { if (!open) setInfo(null) }}><DialogContent className="sm:max-w-lg p-6"><DialogHeader><DialogTitle>{info === 'help' ? 'Safe food. Responsible rescues.' : info === 'notifications' ? 'Your rescue updates' : 'Start a new rescue'}</DialogTitle><DialogDescription>{info === 'help' ? 'Pilot coordination rules, not a substitute for trained food-safety assessment.' : info === 'notifications' ? 'Important changes across your rescue network.' : 'Thank you for turning surplus into possibility.'}</DialogDescription></DialogHeader>{info === 'help' ? <div className="help-content"><p><strong>Never dispatch after the safe window.</strong> Preparation time, temperature, condition, and handling time must all be checked.</p><p><strong>License collected, not verified.</strong> A recorded FSSAI number is not proof of compliance.</p><p><strong>Synthetic means synthetic.</strong> Pilot locations, people, and food records are demonstration inputs. Only synthetic text may go to AI services.</p><a href="https://sharefood.eatrightindia.gov.in/guidance-for-fresh-cooked-food.html" target="_blank" rel="noreferrer">Read IFSA food-safety guidance ↗</a></div> : info === 'notifications' ? <div className="help-content">{urgent?.length ? urgent.map(d => <button className="notification-item" key={d.id} onClick={() => { setSelected(d); setInfo(null) }}><ClockNotice /><span>{d.item} needs attention before its safe window closes.</span><ChevronRight size={15} /></button>) : <p>No urgent rescue alerts to show. Telegram notifications will be enabled only after the bot connection is authorized.</p>}</div> : <div className="help-content"><Alert><Database /><AlertTitle>Database setup approval needed</AlertTitle><AlertDescription>Donation intake will open after the spatial schema and safety service are configured. No unsaved donation will be shown as posted.</AlertDescription></Alert><p>You can create your account now. New accounts receive donor access; coordinator privileges are granted separately.</p><Button onClick={() => { setInfo(null); setAuth(true) }}>{session ? 'Manage sign-in' : 'Sign in or create an account'}</Button></div>}</DialogContent></Dialog>
  </div>
}
