...
export default function App({ data, source, refresh, backend, onBackToLanding }: { data?: PilotData; source?: 'supabase' | 'offline'; refresh?: () => void; backend?: BackendHealth; onBackToLanding?: () => void }) {
  ...
  return <div className="app-shell"><a className="skip-link" href="#main-content">Skip to content</a><AppSidebar section={section} setSection={setSection} mobileOpen={mobile} close={() => setMobile(false)} session={session} signIn={() => setAuth(true)} signOut={signOut} activeCount={active?.length} help={() => setInfo('help')} onBackToLanding={onBackToLanding} /><div className="app-main"><header className="topbar"><div className="breadcrumb"><Button variant="ghost" size="icon" className="mobile-menu" aria-label="Open navigation" onClick={() => setMobile(true)}><Menu /></Button><span>Workspace</span><ChevronRight size={13} /><strong>{sectionNames[section]}</strong></div><div className="topbar-actions"><button className="location-button" onClick={() => setSection('settings')}><MapPin size={14} /><span>Bengaluru, IN</span><ChevronDown size={12} /></button><span className="topbar-divider" /><Button variant="ghost" size="icon" aria-label="Safety guidelines" onClick={() => setInfo('help')}><CircleHelp /></Button><Button variant="ghost" size="icon" aria-label="View notifications" onClick={() => setInfo('notifications')}><Bell /></Button><button className="topbar-avatar" onClick={() => setAuth(true)} aria-label={session ? 'Account' : 'Sign in'}>{session ? 'RP' : <Leaf size={17} />}</button></div></header>
    <main id="main-content" className="page-content"><div className="page-heading"><div><div className="heading-eyebrow"><span className="status-dot" />AAHARSETU · BENGALURU FOOD RESCUE NETWORK</div><h1>{section === 'overview' ? 'Rescue overview' : sectionNames[section]}</h1><p>{descriptions[section]}</p></div><div className="heading-actions"><Badge variant="outline">{source === 'supabase' ? 'Supabase Live' : 'Pilot dataset'}</Badge><Button size="lg" onClick={() => setPosting(true)}><Plus data-icon="inline-start" />Post a donation</Button></div></div>
    ...
    {!data && <div className="setup-banner"><span className="setup-banner-icon"><Database size={16} /></span><p><strong>Connecting to Bengaluru rescue network…</strong><span>Initializing live data stream and spatial network.</span></p><button onClick={() => setSection('settings')}>View setup<ChevronRight size={14} /></button></div>}
    ...
    {section === 'dispatch' && <DispatchView data={data} now={now} openDonation={setSelected} refresh={refresh ?? (() => {})} />}
    ...
    {section === 'settings' && <SettingsView source={source} backend={backend} />}
    ...
    <AuthDialog open={auth} onOpenChange={setAuth} />
    <DonationDialog donation={selected} data={data} now={now} close={() => setSelected(null)} />
    <DonationForm open={posting} data={data} refresh={refresh ?? (() => {})} onClose={() => setPosting(false)} />
